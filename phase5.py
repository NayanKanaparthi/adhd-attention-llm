"""
Phase 5: Wanderer with internal disinhibition (attention dropout).

Same Wanderer/Critic/Synthesis loop as Phase 4, BUT:
- No hardcoded domain prompts. Wanderer gets only a minimal "find an unusual angle" nudge.
- Wanderer's generation uses attention dropout (10%) from Phase 3. The model has to
  find diversity through loose attention, not through external scaffolding.
- Critic and Synthesis run with stock attention (dropout=0).

Direct comparison to Phase 4 tells us whether attention modification produces genuine
creative wandering, or whether Phase 4's diversity came entirely from prompt scaffolding.

Run with: modal run phase5.py
"""
...
app = modal.App("adhd-attention-phase5")
...
# Same prompts as Phase 4 for direct comparison
TEST_PROMPTS = [
...
# Global controller for attention dropout.
# Set this before generation calls. 0.0 = stock attention. 0.10 = Phase 3's sweet spot.
DROPOUT_RATE = 0.0
...
def run_phase5(seed: int = 42, n_candidates: int = 6, wanderer_dropout: float = 0.10):
    import torch
    ...
    # ---------- patched attention forward with controllable dropout ----------
    ...
    def patched_forward(
        ...
        # ---- ADHD intervention: random per-weight dropout, then renormalize ----
        if DROPOUT_RATE > 0.0:
            keep_mask = (torch.rand_like(attn_weights) > DROPOUT_RATE).to(attn_weights.dtype)
    ...
    # ---------- verify patched forward at dropout=0 matches stock ----------
    print("Verifying patched forward at DROPOUT_RATE=0 matches stock attention...")
    DROPOUT_RATE = 0.0
    ...
    # Now generate with patched forward (dropout=0, should be identical)
    torch.manual_seed(seed)
    with torch.no_grad():
    ...
    if stock_text == patched_text:
        print(f"  MATCH: patched forward at dropout=0 is byte-identical to stock.\n")
    else:
        print(f"  MISMATCH!")
    ...
    # ---------- WANDERER: loose attention, minimal nudge, no domain hints ----------
    def wanderer(problem: str, idx: int) -> str:
        ...
        torch.manual_seed(seed + (idx + 1) * 1000)  # avoid collision with baseline seed
        DROPOUT_RATE = wanderer_dropout  # loose attention ON
        result = generate(prompt, max_tokens=200, temperature=1.0, sample=True)
        DROPOUT_RATE = 0.0  # tight attention restored
        return result
    ...
    def critic(problem: str, candidate: str) -> dict:
        ...
        DROPOUT_RATE = 0.0  # tight attention for evaluation
    ...
    for prompt in TEST_PROMPTS:
        ...
        # 2. Wanderer: loose attention, no domain hints
        print(f"\n--- WANDERER ({n_candidates} candidates, attention dropout={wanderer_dropout}, no domain hints) ---")
        candidates = []
